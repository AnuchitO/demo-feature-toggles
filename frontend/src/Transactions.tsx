...
export const TransactionItem = ({ type = "Activity", transferredAt = "18 Jan 2025 15:03", amount = 0 }: TransactionProps) => {
  return <>
    <div className="flex justify-between m-4">
      ...
      <div className="flex flex-col text-right">
        <p className="font-thin text-gray-400 text-xs/6 tracking-tighter font-os">{formatDate(transferredAt)}</p>
    ...
    <div className="w-full h-full min-w-100 min-h-40 rounded-lg shadow-lg" >
      {transactions.map((transaction, index) => {
        return <Fragment key={index}>
          <TransactionItem type={transaction.type} transferredAt={transaction.transferredAt} amount={transaction.amount} />
        </Fragment>
      })}
    </div>
